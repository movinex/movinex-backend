...
export class VerificamexService {
  ...
  static async validarCURP(curp: string, emailCliente?: string): Promise<{ valido: boolean; datos?: any; rawData: any }> {
    try {
      const esPruebaReal = emailCliente && emailCliente.toLowerCase().includes('real');

      if (!this.API_KEY || !esPruebaReal) {
        console.log(`[Verificamex MOCK] Validando CURP localmente (Simulado) para: ${curp}`);
        return {
          valido: true,
          datos: {
            curp,
      ...
      console.log(`[Verificamex] Solicitando validación de CURP ante RENAPO para: ${curp}`);
      const response = await axios.post(
        `${this.BASE_URL}/scraping/renapo`,
        { curp },
        {
          headers: {
            'Authorization': `Bearer ${this.API_KEY}`,
      ...
      const data = response.data;
      console.log('[Verificamex] Respuesta de RENAPO recibida:', JSON.stringify(data));

      const status = data.status || (data.data && data.data.status);
      const esValido = status === 'success' || data.success === true || (data.data && data.data.nombre);
      ...
      if (error.response) {
        console.error('[Verificamex] Error en validación de CURP (RENAPO):', {
          status: error.response.status,
          data: error.response.data
        });
      } else {
        console.error('[Verificamex] Error en validación de CURP (RENAPO):', error.message);
      }
      return {
        valido: false,
        rawData: { error: true, message: error.message }
