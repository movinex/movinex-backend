...
- `verificamexService.ts` — KYC (validación de teléfono + OCR/biometría INE/selfie), vía dos llamadas: `leerDatosINE` (`/ocr/obverse`) + `validarIdentidadBiometrica` (`/validations/compare_face`, booleano `isMatch` sin score). **Go-live (2026-08-10)**: `leerDatosINE`/`validarIdentidadBiometrica` pegan a la API real por default — solo el email exacto `desarrollo@movinex.mx` cae a mock local (antes era al revés: default mock, "real" en el email pasaba a producción; se invirtió para que un cliente real no dependa de que nadie le ponga "real" en el email). `validarTelefono` sigue sin llamar nunca a la API real, siempre aprueba. `POST /api/solicitudes` ahora exige además que el OCR haya devuelto nombre y CURP (cuando no es mock) para auto-aprobar — una foto de INE demasiado mala para leerse ya no se aprueba sola solo porque el biométrico hizo match. Hubo una versión que combinaba OCR + biometría en una sola llamada a `/validations/basic` (daba un score numérico configurable) — se revirtió porque ese endpoint devuelve **500 Server Error consistente** (confirmado con imágenes reales, payload validado 1:1 contra la doc de Verificamex — es un bug de su lado, no nuestro). Reportado a su soporte; mientras tanto quedamos con el booleano ciego de `compare_face`.
...
- **Supabase**: correr manualmente en el SQL editor (no hay CLI/migraciones en el repo):
  ...
  alter table solicitudes add column if not exists costo_envio numeric not null default 0;
  ```
