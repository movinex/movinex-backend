...
- `conektaService.ts` — `crearCheckoutEnganche` genera el link de pago viejo (PaymentLink, sin usar desde el frontend). El cobro real es `crearOrdenEnganche`: crea una Orden con `checkout: { type: "Integration", allowed_payment_methods: ["card"] }` para pagarla con el **Checkout Component** de Conekta (`conekta-checkout.min.js`, iframe hosteado por ellos) — reemplaza al tokenizador clásico `Conekta.js` + `Token.create()`, que esta cuenta rechaza ("the merchant does not accept payments with your payment method"). `crearSuscripcionSemanal` (Plan + Subscription, `trial_period_days: 7`) queda escrito pero **sin usar todavía**: guardar tarjetas para reutilizarlas requiere "Early Access" de Conekta, hay que pedirlo antes de poder armar el customer+tarjeta que esto necesita.
...
- `POST /api/solicitudes/:id/crear-orden-enganche` — crea la Orden en Conekta y devuelve el `checkoutId` que el frontend usa para montar el Checkout Component. No cobra nada por sí sola — el pago lo hace el cliente en el iframe. Reemplaza al viejo `/confirmar-pago-simulado` (ya no existe) y al `/pagar-enganche` basado en tokenId (tampoco existe más).
...
- **Supabase**: correr manualmente en el SQL editor (no hay CLI/migraciones en el repo):
  ...
  ```
- **Conekta**: `VITE_CONEKTA_PUBLIC_KEY` ya está cargada. Falta: (1) que "Tarjetas" esté habilitado como método de pago en el dashboard (Métodos de pago) — con esto ya resuelto el Checkout Component debería andar; (2) pedirle a Conekta Early Access a "Tarjetas Guardadas"/Suscripciones para poder activar `crearSuscripcionSemanal` (hoy no se llama desde ningún lado).
